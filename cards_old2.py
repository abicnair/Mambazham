...
class Cards:
    def __init__(self):
        ...
        # Green cards (adjectives/verbs in Malayalam)
        self.green_cards = [
            {"id": 1, "text": "വലുത്", "translation": "Big"},
            {"id": 2, "text": "ചെറുത്", "translation": "Small"},
            {"id": 3, "text": "നല്ലത്", "translation": "Good"},
            {"id": 4, "text": "ചീത്ത", "translation": "Bad"},
            {"id": 5, "text": "വേഗം", "translation": "Fast"}
        ]

        self.used_red_cards = []
        self.used_green_cards = []

    def draw_red_card(self):
        if not self.red_cards:
            # Reshuffle used cards back into deck
            self.red_cards = self.used_red_cards
            self.used_red_cards = []
            if not self.red_cards:
                return None

        import random
        card = random.choice(self.red_cards)
        self.red_cards.remove(card)
        self.used_red_cards.append(card)
        return card

    def draw_green_card(self):
        if not self.green_cards:
            # Reshuffle used cards back into deck
            self.green_cards = self.used_green_cards
            self.used_green_cards = []
            if not self.green_cards:
                return None

        import random
        card = random.choice(self.green_cards)
        self.green_cards.remove(card)
        self.used_green_cards.append(card)
        return card
